...
def test_get_projekte_returns_list(client):
    response = client.get('/api/projekte')
    assert response.status_code == 200
    assert isinstance(response.get_json(), list)

def test_create_projekt_success(client):
    # Kunde anlegen
    ...
    response = client.post('/api/projekte', json=projekt_data)
...
def test_create_projekt_missing_fields(client):
    # Ohne Projektname
    response = client.post('/api/projekte', json={'kunde_id': 1})
    assert response.status_code == 400
    assert 'error' in response.get_json()
    
    # Ohne Kunde-ID
    response = client.post('/api/projekte', json={'projektname': 'Test Projekt'})
    assert response.status_code == 400
    assert 'error' in response.get_json()

def test_create_projekt_invalid_kunde(client):
...
def test_get_projekte_after_create(client):
    # Kunde anlegen
    kunde_response = client.post('/api/kunden', json={'name': 'Multi Projekt Kunde'})
    kunde_id = kunde_response.get_json()['id']
    ...
    for projekt in projekte_data:
        response = client.post('/api/projekte', json=projekt)
        assert response.status_code == 201
...
def test_get_projekt_detail_success(client, sample_data):
...
def test_get_projekt_detail_not_found(client):
    response = client.get('/api/projekte/999')
    assert response.status_code == 404
    assert 'error' in response.get_json()

def test_create_projekt_empty_body(client):
    response = client.post('/api/projekte', json={})
    assert response.status_code == 400