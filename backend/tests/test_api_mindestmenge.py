...
def test_create_artikel_with_mindestmenge(client, sample_data):
...
def test_create_artikel_default_mindestmenge(client, sample_data):
...
def test_get_artikel_detail_includes_mindestmenge(client, sample_data):
    ...
    response = client.get('/api/artikel/DET-001')
    assert response.status_code == 200
    
    data = response.get_json()
    assert data['mindestmenge'] == 15

def test_get_artikel_unter_mindestmenge_empty(client):
    """Test minimum quantity report when no articles are below minimum"""
    response = client.get('/api/berichte/mindestmenge')
    assert response.status_code == 200
    assert response.get_json() == []

def test_get_artikel_unter_mindestmenge_with_data(client, sample_data):
...
def test_artikel_above_mindestmenge_not_in_report(client, sample_data):
...
def test_artikel_at_mindestmenge_not_in_report(client, sample_data):
...
def test_mindestmenge_after_sales(client, sample_data):
...
def test_multiple_artikel_under_mindestmenge(client, sample_data):
